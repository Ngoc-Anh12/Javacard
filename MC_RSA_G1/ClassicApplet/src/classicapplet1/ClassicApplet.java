...
public class ClassicApplet extends Applet {
    ...
    private void doRSACipher(APDU apdu, short mode, byte[] arr, short len, short off) {//mode la che do ma hoa hay giai ma
        ...
        //RSA encrypt, Public Key se duoc su dung
        if (mode == (short) 0) {
            //khoi tao public key 
    ...
    //Nhận giá trị của Khóa công khai RSA từ biến toàn cục 'rsaPubKey', p1 = 0 lay ra N, p1= 1 lay ra E
    ...
    private void genRsaKeyPair(APDU apdu) {
        byte[] buffer = apdu.getBuffer();
        try {
            //khoi tao khoa de tao ra cap khoa
            ...
            //tao ra khoa cong khai tu khoa vua dc tao
            RSAPublicKey pubKey = (RSAPublicKey) keyPair.getPublic();
            short pubKeyLen = 0;
            //luu tru gia tri khoa cong khai rsa trong bien toan cuc rsapubkey, khoa cong khai chua module n va exponent e
           
            pubKeyLen += pubKey.getModulus(rsaPubKey, pubKeyLen);//N
            pubKeyLen += pubKey.getExponent(rsaPubKey, pubKeyLen);//E

            short priKeyLen = 0;
            //tra ve mot tham chieu den thanh phan khoa rieng cua doi tuong keypair
            
            RSAPrivateKey priKey = (RSAPrivateKey) keyPair.getPrivate();
            //thuat toan RSA, khoa rien chua n va d, luu tru cac gia tri tham so nay trong bien toan cu rsaRrikey
...
//duoc goi boi applet de kich hoat dich vu xoa doi tuong cua JCRE
        JCSystem.requestObjectDeletion();
    }
    ...
    private void doAesCipher(APDU apdu, byte[] arr, short len, byte mod, byte[] arrb) {
        try {
            byte[] buffer = apdu.getBuffer();
            //getInstance cung cap hai tham so de tao mot doi tuong cipher
            //tham so externalAccess-->cho biet doi tuong dc tra ve co the truy cap ben ngoai hay khong
            aesEcbCipher = Cipher.getInstance(Cipher.ALG_AES_BLOCK_128_CBC_NOPAD, false);//externalAccess  =false
            //tham so chi dinh mot thuat toan mat ma
            ...
            //khoi tao doi tuong cipher bang 1 khoa thich hop va chi dinh dinh xem khoa do dc su dung de ma hoa hay giai ma
